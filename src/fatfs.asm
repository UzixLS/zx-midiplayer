...
BS_FilSysType32  equ 82   ; len 8    Always "FAT32   " and has not any effect in determination of FAT type.
...
.BPB_SecPerClus:
    ...
    ld a, (disk_buffer+BPB_SecPerClus)            ; save BPB_SecPerClus * multiplier (internal disk sector size is always 512-byte)
    dec b                                         ; ...
    jr z, 2f                                      ; ...
1:  add a, a                                      ; ...
    jp c, .err                                    ; ... overflow?
    djnz 1b                                       ; ...
2:  ld (var_disk.fatfs.BPB_SecPerClus), a         ; ...
...
.BPB_NumFATs:
    ...
    ret                                           ; ...
.BPB_RootEntCnt:
    ld hl, (disk_buffer+BPB_RootEntCnt)           ; check number of entries in root directory
    ld a, h                                       ; ...
    or l                                          ; ...
    jr nz, .fat16                                 ; ...
.fat32:
.BS_FilSysType32:
    ld hl, disk_buffer+BS_FilSysType32            ; check expected string
    ld a, 'F' : cpi : ret nz                      ; ...
    ld a, 'A' : cpi : ret nz                      ; ...
    ld a, 'T' : cpi : ret nz                      ; ...
    ld a, '3' : cpi : ret nz                      ; ...
...
.fat16:
.BS_FilSysType16:
    ld hl, disk_buffer+BS_FilSysType16            ; check expected string
    ld a, 'F' : cpi : ret nz                      ; ...
    ld a, 'A' : cpi : ret nz                      ; ...
    ld a, 'T' : cpi : ret nz                      ; ...
    ld a, '1' : cpi : ret nz                      ; ...
    ld a, '6' : cpi : ret nz                      ; ...
.BPB_TotSec:
    ld hl, (disk_buffer+BPB_TotSec16)             ; check total number of sectors != 0
    ld a, h                                       ; ...
    or l                                          ; ...
    jr nz, .BPB_FATSz16_fat16                     ; ...
    ld hl, (disk_buffer+BPB_TotSec32+0)           ; ...
    ld a, h                                       ; ...
    or l                                          ; ...
    ld hl, (disk_buffer+BPB_TotSec32+2)           ; ...
    or h                                          ; ...
    ...
    jr z, .err                                    ; ...
.BPB_FATSz16_fat16:
    ld hl, (disk_buffer+BPB_FATSz16)              ; check FAT16 size != 0
    ld a, h                                       ; ...
    or l                                          ; ...
    jr z, .err                                    ; ...
.exit_ok:
...
.get_root_directory:
    ld hl, 0                                      ;
    xor a                                         ;
    ld (var_disk.fatfs.BPB_RootSecCnt), a         ;
    ld hl, (disk_buffer+BPB_RootEntCnt)           ;
    ld a, h                                       ;
    or l                                          ;
    jr nz, .get_fat16_root_directory              ;
.get_fat32_root_directory:
    ...
    jr .set_disk_rw_base_lba                      ;
.get_fat16_root_directory:
    ld de, 15                                     ; BPB_RootSecCnt = (BPB_RootEntCnt * 32 + 511) / 512 // 511 for ceil
    add hl, de                                    ; ... = (BPB_RootEntCnt + 15) / 16 // 15 for ceil
    ld a, l                                       ; ...
    srl h : rra                                   ; ...
    srl h : rra                                   ; ...
    srl h : rra                                   ; ...
...
.calc_data_lba:
    ld a, (var_disk.fatfs.BPB_RootSecCnt)         ;
    or a                                          ;
    jr nz, .calc_fat16_data_lba                   ;
...
1:  ld (var_disk.fatfs.data_lba+0), de            ;
    ...
    jr .exit_ok                                   ;
.calc_fat16_data_lba:
    ld de, (disk_buffer+BPB_FATSz16)              ;
    ld hl, 0                                      ;
    ld a, (disk_buffer+BPB_NumFATs)               ; 2xfat?
    dec a                                         ; ...
    jr z, 1f                                      ; ...
    sla e : rl d : rl l : rl h                    ; ...
1:  ld (var_disk.fatfs.BPB_RootClus+0), de        ; root directory lba
    ld (var_disk.fatfs.BPB_RootClus+2), hl        ;
    ld a, (var_disk.fatfs.BPB_RootSecCnt)         ;
    add a, e                                      ;
    ld e, a                                       ;
...
fatfs_get_lba:
    push hl                                    ;
.fat16_root_dir:
    ld a, d : or e : or b : or c               ; cluster_n == 0 means fixed FAT16 root directory
    jr nz, .sub_0x00000002                     ; ...
    ld a, (var_disk.fatfs.BPB_RootSecCnt)      ; if BPB_RootSecCnt == 0 - error
    or a                                       ; ...
    jr z, .err_pop                             ; ...
    pop hl                                     ; if sector offset > 255 - error
    ld a, h                                    ; ...
    or a                                       ; ...
    jr nz, .err                                ; ...
    ld a, (var_disk.fatfs.BPB_RootSecCnt)      ; if sector offset >= BPB_RootSecCnt - error
    cp l                                       ; ...
    jr z, .err                                 ; ...
    jr c, .err                                 ; ...
    ld bc, (var_disk.fatfs.BPB_RootClus+0)     ; lba = root directory lba + sector offset
...
.fat32:
    ...
    ret                                        ;
.fat16:
    ld a, d : or e                             ; if cluster_n > 0xffff - error
    jr nz, .err                                ; ...
    push bc                                    ;
    assert disk_sector_size == 512
    ld c, b                                    ; lba of fat sector = cluster_n * 2 / 512
    ld b, 0                                    ;
    ld de, 0                                   ;
    ld hl, fat_buffer                          ; get corresponding fat sector
    ld ixl, 1                                  ; ...
    ...
    pop bc                                     ;
    ret nz                                     ; ...
    ld b, 0                                    ; each 512-byte sector in fat16 contains information about 256 clusters
    or a                                       ; ... and each entry is 2 byte long
    rl c : rl b                                ; ...
    ld hl, fat_buffer                          ; ...
    add hl, bc                                 ; ...
    ld c, (hl) : inc hl : ld b, (hl)           ; get info from fat about next cluster in chain
    ld de, 0                                   ; ...
    xor a                                      ; set Z flag
    ret                                        ;
.err:
...
.fat16_root_dir:
    ld a, (var_disk.fatfs.BPB_RootSecCnt)           ; in fat16 root directory doesn't use clusters
    or a                                            ; ...
    jr z, .clustered_dir                            ; ...
    ld a, d : or e : or b : or c                    ; ...
    jr nz, .clustered_dir                           ; ...
    ld a, (var_disk.fatfs.BPB_RootSecCnt)           ; if we already at the last root dir sector - exit
    dec a                                           ; ...
    cp l                                            ; ...
    jr nz, .next_sector                             ; ...
    or 1                                            ; ... set NZ flag
...
.fat16_root_dir:
    ...
    ld a, d : or e : or b : or c                    ; ...
    jr nz, .clustered_dir                           ; ...
    ld a, (var_fatfs.dir_window_sector_pos)         ; if we already at the first root dir sector - exit
    or a                                            ; ...
    jr nz, .prev_sector                             ; ...
    or 1                                            ; ... set NZ flag
    ret                                             ; ...
